...
function Studio(runtime, element) {
    ...
    $(element).find('.save-button').click(function (eventObject) {
        ...
        const editorContent = $('#tinymce', element).val() 
    ...
    tinymce.PluginManager.add('answerinput', function(editor, url) {
        var input = `<input placeholder="answer here" style="margin-left: 1em; margin-right: 1em"/>`
  ...
  editor.ui.registry.addMenuItem('answerinput', {
    ...
    tinymce.PluginManager.add('sourcecode', function(editor, url) {
        const textarea = `<textarea placeholder="Source code here ..." style="width: 100%;height: 20em; margin-top: 1em; margin-bottom: 1em;"></textarea>`
  ...
  editor.ui.registry.addMenuItem('sourcecode', {
    ...
    onAction: function() {
      ...
      tinymce.init({
        ...
        if(e.keyCode == 13 && e.target.id != "tinymce"){
            e.target.value = e.target.value + "\n"
            e.preventDefault()
        }
        if(e.keyCode == 9 && e.target.id != "tinymce"){
            e.target.value = e.target.value + "\t"
            e.preventDefault()
        }
    });
